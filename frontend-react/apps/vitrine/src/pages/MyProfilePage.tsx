...
import { Button, Card, FormField, TextInput } from '@bs/ui';
import { useAuth } from '@bs/auth';
import { requestPasswordReset } from '@bs/api-client';
import { AccountShell, useUpdateProfile, readStoredFirstName } from '../features/account';
...
export function MyProfilePage() {
  ...
  const [firstName, setFirstName] = useState(readStoredFirstName());
    ...
    <AccountShell title="Mon profil">
      <Card>
        <form className="bs-acc-form" onSubmit={onSubmit}>
          ...
          <FormField label="Adresse e-mail">
            <TextInput value={user?.email ?? ''} readOnly disabled autoComplete="email" />
          </FormField>
      ...
      <Card>
        <div className="bs-acc-form">
          ...
          <div className="bs-acc-form__actions">
            <Button variant="secondary" onClick={onResetPassword} disabled={!user?.email}>
