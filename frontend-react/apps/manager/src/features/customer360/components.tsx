...
// ── Carte recherche client ──────────────────────────────────────────────────
export function ClientSearchCard({ card, onOpen }: { card: CustomerSearchCard; onOpen: (id: string) => void }) {
  return (
    <button type="button" className="c3-card c3-clientcard" onClick={() => onOpen(card.id)} data-testid="c3-clientcard">
      <span className="c3-clientcard__avatar" aria-hidden="true">{initials(card.firstName, card.lastName, '🙂')}</span>
      <span className="c3-clientcard__body">
        <span className="c3-clientcard__name">{card.displayName}{card.bookingSuspended ? <span className="c3-badge c3-badge--suspended">Suspendu</span> : null}</span>
        <span className="c3-clientcard__sub">Dernière visite {fmtDate(card.lastVisitAt)}</span>
        {card.nextBookingAt ? <span className="c3-clientcard__next"><i className="bi-calendar-check" aria-hidden="true" /> {fmtDateTime(card.nextBookingAt)}</span> : null}
      </span>
      <i className="bi-chevron-right c3-clientcard__chev" aria-hidden="true" />
    </button>
  );
}
